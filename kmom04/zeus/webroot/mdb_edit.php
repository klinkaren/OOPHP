...
foreach($res AS $key => $val) {
  $tr .= "<tr><td>{$key}</td><td>{$val->id}</td><td><img width='80' height='40' src='{$val->image}' alt='{$val->title}' /></td><td>{$val->title}</td><td>{$val->YEAR}</td><td class='menu'><a href='mdb_edit_selected.php?id={$val->id}'><i class='icon-edit'></i></a></td></tr>";
}
